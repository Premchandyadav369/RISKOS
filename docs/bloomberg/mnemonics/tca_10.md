# Bloomberg Terminal Function: <TCA> (Part 10)

## Transaction Cost Analysis & Implementation Shortfall

### Description
Perold 4-way frictional drag decomposition.

### Institutional Bloomberg Workflow
1. Type `<TCA>` into the RISKOS Command Line Bar (`⌘K` or `F1`-`F12`).
...
### KaTeX Formulation
$$\text{TCA}_{t} = \arg\min_{\mathbf{w} \in \mathcal{W}} \mathcal{L}(\mathbf{w}, \mathbf{\Sigma}, \boldsymbol{\mu})$$
