# Bloomberg Terminal Function: <GARCH> (Part 10)

## GARCH(1,1) Volatility Clustering Engine

### Description
Bollerslev conditional variance maximum likelihood estimation.

### Institutional Bloomberg Workflow
1. Type `<GARCH>` into the RISKOS Command Line Bar (`⌘K` or `F1`-`F12`).
2. Set asset class parameters and historical lookback horizon.
3. Review computed risk metrics, LaTeX analytical formulation, and trade execution blotter.

### KaTeX Formulation
$$\text{GARCH}_{t} = \arg\min_{\mathbf{w} \in \mathcal{W}} \mathcal{L}(\mathbf{w}, \mathbf{\Sigma}, \boldsymbol{\mu})$$

*RISKOS Institutional Bloomberg Terminal Core Engine Specification v3.5.*
